...
function joinRoom(socket, room) {
    ...
    socket.emit(STATIC_TEXT.JOIN_RESULT, { room: room }); //让用户知道他们进入了新的房间
    socket.broadcast.to(room).emit(STATIC_TEXT.MESSAGE, {  //让房间里的所有人知道有人进入房间
        text: nickNames[socket.id] + ' 加入了 ' + room + '.'
    });

    console.log(room);
    var usersInRoom = io && io.of('/').in(room).clients; //有多少用户在房间
    if (usersInRoom.length > 1) {
        var usersInRoomSummary = '目前房间 ' + room + '人数有: ';
        ...
        usersInRoomSummary += '.';
        socket.emit(STATIC_TEXT.MESSAGE, { text: usersInRoomSummary }); //汇总该房间里的其它成员名称发送给给该用户
    }
}
